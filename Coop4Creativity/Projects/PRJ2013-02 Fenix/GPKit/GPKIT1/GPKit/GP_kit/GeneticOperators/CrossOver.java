/* -------------------------------------------------------------------------- */
/* Classe    : CrossOver                                                      */
/* Descrição : Classe para a implementação do operador genético CrossOver.    */
/* -------------------------------------------------------------------------- */
...
/* ------------------------- constantes de classe --------------------------- */
  ...
  static final double DEFAULT_crossover_external = 0.1;    
...
public CrossOver(selectMethod s1)
  {  this(s1,
          ...
          DEFAULT_crossover_external);  }

/* ------------------------- variáveis de instânçia ------------------------- */
  
  /* método de selecção do primeiro progenitor. */
  private selectMethod select1;
  /* método de selecção do segundo progenitor. */
  private selectMethod select2;
  /* especifica o que fazer quando um crossover produz uma árvore que viola
    ...
    do
      {
        ...
        /* aplicar a transformação no primeiro progenitor */
        if (cross_point_p1 == 0)
            parent1 = subtree2;
        else    
            parent1.replaceTree(cross_point_p1-1,subtree2);
        
        /* aplicar a transformação no segundo progenitor */    
        if (cross_point_p2 == 0)
           parent2 = subtree1;
        else    
    ...
    if (o instanceof CrossOver)
      {
        ...
        equal = this.getName().equals(cross_aux.getName()) &&
                ...
                (this.getExternal() == cross_aux.getExternal());
      }
    return (equal);
  }
